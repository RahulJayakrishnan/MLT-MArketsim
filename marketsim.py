...
def compute_portvals(orders_file = "./orders/orders-02.csv", start_val = 1000000, commission=9.95, impact=0.005):
    ...
    for index, row in orders_df.iterrows():
        if row['Order']=='BUY':
            df_trades.at[index,row['Symbol']]=df_trades.at[index,row['Symbol']]+row['Shares']
            df_trades.at[index,'Cash']=df_trades.at[index,'Cash']-(row['Shares']*df_prices.at[index,row['Symbol']])*(1+impact)-commission

        else:
            df_trades.at[index, row['Symbol']] =df_trades.at[index,row['Symbol']] -row['Shares']
            df_trades.at[index, 'Cash'] = df_trades.at[index, 'Cash'] + row['Shares'] * df_prices.at[
                index, row['Symbol']]*(1-impact)-commission
